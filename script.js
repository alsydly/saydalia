(function() {
  ...
  localStorage.setItem = function(key, value) {
    try {
      ...
      console.warn("localStorage full, using memory fallback for", key);
    }
    window.__memStore[key] = value;
  };
  localStorage.getItem = function(key) {
    if (window.__memStore.hasOwnProperty(key)) {
      return window.__memStore[key];
    }
...
const FIRESTORE_REST_BASE =
  "https://firestore.googleapis.com/v1/projects/ffgfd-78e21/databases/(default)/documents";
...
function readFirestoreDocument(docData) {
  return {
    id: docData.name.split("/").pop(),
    ...readFirestoreFields(docData.fields),
  };
}

async function fetchFirestoreCollection(collectionName) {
  const response = await fetch(`${FIRESTORE_REST_BASE}/${collectionName}?pageSize=50`);
  if (!response.ok) {
    throw new Error(`Firestore REST ${collectionName}: ${response.status}`);
  }
  const data = await response.json();
  return (data.documents || []).map(readFirestoreDocument);
}

async function fetchFirestoreProductsLite() {
  const response = await fetch(`${FIRESTORE_REST_BASE}:runQuery`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`Firestore REST products query: ${response.status}`);
  }
  ...
  return data
    ...
    .map((item) => ({
      ...
      ...readFirestoreDocument(item.document),
    }));
}

async function fetchFirestoreDocument(documentPath) {
  const response = await fetch(`${FIRESTORE_REST_BASE}/${documentPath}`);
...
async function syncDataWithFirestoreRest() {
  try {
    console.log("Fetching data from Firestore REST fallback...");
    const [newProducts, newCats, bannersDoc] = await Promise.all([
      fetchFirestoreProductsLite(),
      fetchFirestoreCollection("categories"),
      fetchFirestoreDocument("meta/banners"),
    ]);

    const newBanners = Array.isArray(bannersDoc?.data) ? bannersDoc.data : [];

    localStorage.setItem("products", JSON.stringify(newProducts));
    localStorage.setItem("categories", JSON.stringify(newCats));
    localStorage.setItem("banners", JSON.stringify(newBanners));
    localStorage.setItem("metaVersion", Date.now().toString());
...
let isProductsLoading = products.length === 0;
...
function syncWhenFirebaseReady(attempt = 0) {
  ...
  if (attempt < 40) {
    setTimeout(() => syncWhenFirebaseReady(attempt + 1), 250);
...
function renderProducts() {
  ...
  if (isProductsLoading && products.length === 0) {
    productsList.innerHTML =
      '<div style="grid-column: 1/-1; text-align: center; padding: 2rem; color: #64748b;">جاري تحميل المنتجات...</div>';
    return;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function prepareInstallModal() {
    if (!installModal) return;

    installModal.classList.add("install-modal");
    installModal.setAttribute("aria-hidden", "true");
    installModal.innerHTML = `
      <div class="install-card" role="dialog" aria-labelledby="install-title" aria-describedby="install-desc">
        <div class="install-icon-wrap">
          <img src="./icon-512x512.png" alt="" class="install-icon" onerror="this.style.display='none'" />
        </div>
        <h3 id="install-title">ثبت التطبيق وخلي الصيدلية أقرب إلك</h3>
        <p id="install-desc">
          احفظ صيدلية السرور على شاشة جهازك لطلب أسرع، وصول أسهل، وتجربة أجمل بدون البحث عن الرابط كل مرة.
        </p>
        <p class="install-note" id="install-fallback-note" hidden>
          إذا لم تظهر نافذة التثبيت، افتح قائمة المتصفح واختر إضافة إلى الشاشة الرئيسية.
        </p>
        <div class="install-actions">
          <button type="button" id="btn-install-app" class="install-primary">تثبيت</button>
          <button type="button" id="btn-skip-install" class="install-secondary">تخطي</button>
        </div>
      </div>
    `;
  }
  ...
  function showInstallModal() {
    ...
    const isLoginVisible =
      loginModal && getComputedStyle(loginModal).display !== "none";

    if (
      !installModal ||
      isLoginVisible ||
      localStorage.getItem("pwaSkipped") === "true" ||
  ...
  window.addEventListener("appinstalled", () => {
    ...
    localStorage.removeItem("pwaSkipped");
  ...
  const installFallbackNote = document.getElementById("install-fallback-note");
  ...
  function prepareLoginModal() {
    ...
    loginModal.className = "modal-overlay login-overlay";
    loginModal.removeAttribute("style");
    loginModal.innerHTML = `
      <div class="login-card">
        <img src="./icon-512x512.png" alt="" class="login-logo" />
        <div class="login-heading">
          <h1>صيدلية السرور النموذجية</h1>
          <p>أهلاً بيك، سجل معلوماتك حتى نجهز طلبك بشكل أسرع.</p>
        </div>
        <form id="login-form" class="login-form">
          <label class="login-field" for="login-name">
            ...
            <span class="login-field-text">الاسم</span>
            <input type="text" id="login-name" placeholder="اكتب اسمك الكريم" required />
          </label>

          <label class="login-field" for="login-address">
            <span class="login-field-icon">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <span class="login-field-text">العنوان</span>
            <input type="text" id="login-address" placeholder="المنطقة، الشارع، أقرب نقطة دالة" required />
          </label>

          <label class="login-field" for="login-phone">
            <span class="login-field-icon">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M22 16.92v3a2 2 0 0 1-2.18 2A19.8 19.8 0 0 1 3.08 5.18 2 2 0 0 1 5.06 3h3a2 2 0 0 1 2 1.72c.12.9.33 1.77.62 2.6a2 2 0 0 1-.45 2.11L9 10.67a16 16 0 0 0 4.33 4.33l1.24-1.24a2 2 0 0 1 2.11-.45c.83.29 1.7.5 2.6.62A2 2 0 0 1 22 16.92Z"></path>
              </svg>
            </span>
            <span class="login-field-text">رقم الهاتف</span>
            <input
              type="tel"
              id="login-phone"
          ...
          <button type="submit" class="login-submit">دخول للمتجر</button>
        </form>
      </div>
    `;
  }
